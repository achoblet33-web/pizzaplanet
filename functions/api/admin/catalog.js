...
async function catalog(db) {
  ...
  const { results: ingredients } = await db.prepare(`SELECT id,name,unit,quantity,low_threshold,out_of_stock_confirmed,out_of_stock_confirmed_at FROM ingredients ORDER BY name`).all();
  const { results: recipes } = await db.prepare(`SELECT product_id,ingredient_id,quantity FROM recipes`).all();
  return { products: results.map(p=>({...p,active:!!p.active,available:!!p.available,variants:variants.filter(v=>v.product_id===p.id).map(v=>({...v,active:!!v.active})),recipe:recipes.filter(r=>r.product_id===p.id)})), ingredients: ingredients.map(i=>({...i,out_of_stock_confirmed:!!i.out_of_stock_confirmed})) };
}
...
export async function onRequest(context) {
  ...
  try {
    if(context.request.method==='POST') {
      ...
      if(input.type==='ingredient') { if(!input.name?.trim()) return json({error:'Nom requis'},400); const restaurant=await db.prepare('SELECT id FROM restaurants ORDER BY id LIMIT 1').first(); const r=await db.prepare(`INSERT INTO ingredients (restaurant_id,name,unit,quantity,low_threshold,out_of_stock_confirmed) VALUES (?,?,?,?,?,0)`).bind(restaurant?.id||1,input.name.trim(),input.unit||'g',Number(input.quantity)||0,Number(input.low_threshold)||0).run(); return json({id:r.meta.last_row_id},201); }
    ...
    if(input.type==='ingredient') await db.prepare(`UPDATE ingredients SET name=?,unit=?,quantity=?,low_threshold=?,out_of_stock_confirmed=CASE WHEN ? > 0 THEN 0 ELSE out_of_stock_confirmed END,out_of_stock_confirmed_at=CASE WHEN ? > 0 THEN NULL ELSE out_of_stock_confirmed_at END WHERE id=?`).bind(input.name,input.unit,Number(input.quantity)||0,Number(input.low_threshold)||0,Number(input.quantity)||0,Number(input.quantity)||0,input.id).run();
    return json({ok:true});
