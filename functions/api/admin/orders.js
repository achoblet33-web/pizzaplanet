...
async function applyStock(db, orderId, direction, reason) {
  ...
  await db.batch(statements);return {ok:true,changed:true};
}

export async function onRequest(context){
 const db=context.env.DB;
 if(context.request.method==='GET'){
  const {results}=await db.prepare(`SELECT id,customer_name,customer_phone,fulfillment_type,total_cents,status,payment_status,notes,created_at,updated_at FROM orders ORDER BY created_at DESC LIMIT 100`).all();return json({orders:results});
 }
 ...
 const order=await db.prepare(`SELECT id,status FROM orders WHERE id=?`).bind(input.id).first();if(!order)return json({error:'Commande introuvable'},404);if(order.status===input.status)return json({ok:true,stock:'unchanged'});
 const lastMovement=await db.prepare(`SELECT reason FROM inventory_movements WHERE order_id=? ORDER BY id DESC LIMIT 1`).bind(input.id).first();
 const wasConsumed=lastMovement?.reason==='order_consumption';const shouldConsume=CONSUME_AT.has(input.status);
 if(shouldConsume&&!wasConsumed){
  const result=await applyStock(db,input.id,-1,'order_consumption');
  if(!result.ok)return json({error:'Stock insuffisant pour préparer cette commande',insufficient:result.insufficient.map(x=>({ingredient_id:x.ingredient_id,name:x.name,stock:Number(x.stock_qty),required:Number(x.required_qty),unit:x.unit}))},409);
 }
 if(input.status==='cancelled'&&wasConsumed)await applyStock(db,input.id,1,'order_restoration');
