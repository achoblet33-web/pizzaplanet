...
async function consumeDoughAndQueue(db,orderId){
 ...
 if(Number(order.stock_deducted)===1){await db.prepare(`INSERT OR IGNORE INTO print_jobs(order_id,status) VALUES(?,'pending')`).bind(orderId).run();return {ok:true,changed:false}}
 ...
 statements.push(db.prepare(`INSERT OR IGNORE INTO print_jobs(order_id,status,created_at) VALUES(?,'pending',?)`).bind(orderId,now));
...
export async function onRequest(context){
 ...
 try{
  ...
  if(orderId&&(event.type==='checkout.session.completed'||event.type==='checkout.session.async_payment_succeeded')){
   ...
   if(paid){await context.env.DB.prepare(`UPDATE orders SET payment_status='paid',updated_at=? WHERE id=?`).bind(new Date().toISOString(),orderId).run();const stock=await consumeDoughAndQueue(context.env.DB,orderId);if(!stock.ok){await context.env.DB.prepare(`UPDATE stripe_events SET status='stock_error',processed_at=? WHERE event_id=?`).bind(new Date().toISOString(),event.id).run();return json({received:true,warning:stock.error,insufficient:stock.insufficient||[]})}}
