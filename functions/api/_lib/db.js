...
export async function listProducts(env){
 ...
 const {results:capacity}=await env.DB.prepare(`SELECT r.product_id, MIN(CASE WHEN r.quantity>0 THEN i.quantity/r.quantity ELSE 999999 END) capacity FROM recipes r JOIN ingredients i ON i.id=r.ingredient_id GROUP BY r.product_id`).all();
 const cap=new Map(capacity.map(x=>[Number(x.product_id),Number(x.capacity)]));
 return results.map(p=>{const c=cap.has(Number(p.id))?cap.get(Number(p.id)):999999;return {...p,available:Boolean(p.available)&&c>=1,stock_capacity:c,variants:variants.filter(v=>v.product_id===p.id).map(v=>({...v,active:Boolean(v.active)}))}});
}
