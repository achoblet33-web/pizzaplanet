...
export async function listProducts(env){
 ...
 return results.map(p=>({...p,available:Boolean(p.available),stock_capacity:null,stock_blocked_by_confirmation:false,variants:variants.filter(v=>v.product_id===p.id).map(v=>({...v,active:Boolean(v.active)}))}));
}
